...
window.makeVideoPlayerHtml = function (path, storage, height, tagId) {
  ...
  if (storage === 'youtube' || storage === 'vimeo') {
    ...
    //   alert(storage);
      if (storage === "iframe") {
           html = '<div>'+path+'</div>';
               var $modal1 = $('#playVideo');
    var $modalLoading1 = $modal1.find('.file-video-loading');
    var $modalVideoContent1 = $modal1.find('.js-modal-video-content');
           $modalLoading1.addClass('d-none');
      $modalVideoContent1.removeClass('d-none');
      $modal1.find('.modal-video-item').removeClass('active');
      }else{
