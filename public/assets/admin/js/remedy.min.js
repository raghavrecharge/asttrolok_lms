...
console.log(data);
    if (s3Input && s3Input.prop('refiles') && s3Input.prop('refiles')[0]) {
      formData.append('s3_file', s3Input.prop('refiles')[0]);
    }
...
//   console.log('pratul...........',value);
// }
          console.log('pratul',result);
        //   alert(result);
  ...
  window.handleRemedyItemForm = function (form, $this) {
  ...
  $('body').on('click', '.save-chapter', function (e) {
    ...
    handleRemedyItemForm(form, $this);
  });
  $('body').on('click', '.js-add-chapter', function (e) {
    var $this = $(this);
    var remedyId = $this.attr('data-remedy-id');
    ...
    Swal.fire({
      ...
      onOpen: function onOpen() {
        ...
        modal.find('input.js-chapter-remedy-id').val(remedyId);
        ...
        if (itemId) {
          ...
          $.get(adminPanelPrefix + '/chapters/' + itemId + '?locale=' + locale, function (result) {
            if (result && result.chapter) {
              ...
              localeSelect.addClass('js-remedy-content-locale');
  ...
  * */
  ...
  $('body').on('click', '#remedyAddFAQ', function (e) {
    e.preventDefault();
    var add_faq_modal = '<div id="addFAQsModal">';
    add_faq_modal += $('#remedyFaqModal').html();
    add_faq_modal += '</div>';
    Swal.fire({
      html: add_faq_modal,
      showCancelButton: false,
  ...
  * */
  ...
  function editExtraDescription($this, locale) {
    ...
    var remedy_id = $this.attr('data-remedy-id');
    var rendomKey = randomString();
    var edit_data = {
      item_id: remedy_id,
      locale: locale
    };
    $.post(adminPanelPrefix + '/remedy-extra-description/' + item_id + '/edit', edit_data, function (result) {
      if (result && result.remedyExtraDescription) {
        var remedyExtraDescription = result.remedyExtraDescription;
        var html = '<div id="extraDescriptionModal">';
        html += $('#extraDescriptionForm').html();
        html += '</div>';

        if (remedyExtraDescription.type === 'company_logos') {
          html = handleCompanyLogosInputHtml(rendomKey);
        }

        html = html.replaceAll(adminPanelPrefix + '/remedy-extra-description/store', adminPanelPrefix + '/remedy-extra-description/' + item_id + '/update');
        Swal.fire({
          html: html,
          showCancelButton: false,
          showConfirmButton: false,
          ...
          onOpen: function onOpen() {
            ...
            Object.keys(remedyExtraDescription).forEach(function (key) {
              $modal.find('[name="' + key + '"]').val(remedyExtraDescription[key]);
            });
            ...
            if (localeSelect) {
              ...
              localeSelect.attr('data-remedy-id', remedy_id);
            }
          }
        });
      }
  ...
  function editTicket($this) {
    ...
    $.post(adminPanelPrefix + '/tickets/' + ticket_id + '/edit', edit_data, function (result) {
      if (result && result.ticket) {
        ...
        edit_ticket_modal += $('#remedyTicketModal').html();
  ...
  function editSession($this) {
    ...
    $.post(adminPanelPrefix + '/sessions/' + session_id + '/edit', edit_data, function (result) {
      if (result && result.session) {
        ...
        edit_session_modal += $('#remedySessionModal').html();
        ...
        Swal.fire({
          ...
          onOpen: function onOpen() {
            ...
            if (localeSelect) {
              ...
              localeSelect.attr('data-remedy-id', webinar_id);
            }
          }
        });
      }
  ...
  function editFaq($this) {
    ...
    $.post(adminPanelPrefix + '/faqs/' + faq_id + '/edit', edit_data, function (result) {
      if (result && result.faq) {
        ...
        edit_faq_modal += $('#remedyFaqModal').html();
        edit_faq_modal += '</div>';
  ...
  * */
  ...
  $('body').on('click', '.edit-remedy-quiz', function (e) {
    e.preventDefault();
    var $this = $(this);
    var remedy_quiz_id = $this.attr('data-remedy-quiz-id');
    var remedy_id = $this.attr('data-remedy-id');
    loadingSwl();
    var edit_data = {
      item_id: remedy_id
    };
    $.post(adminPanelPrefix + '/remedy-quiz/' + remedy_quiz_id + '/edit', edit_data, function (result) {
      var _this4 = this;

      if (result && result.remedyQuiz) {
        var remedy_quiz = result.remedyQuiz;
        var edit_remedy_quiz_modal = '<div id="addQuizModal">';
        edit_remedy_quiz_modal += $('#quizzesModal').html();
        edit_remedy_quiz_modal += '</div>';
        edit_remedy_quiz_modal = edit_remedy_quiz_modal.replaceAll(adminPanelPrefix + '/remedy-quiz/store', adminPanelPrefix + '/remedy-quiz/' + remedy_quiz_id + '/update');
        edit_remedy_quiz_modal = edit_remedy_quiz_modal.replaceAll('quiz-select2', 'quiz-select22');
        Swal.fire({
          html: edit_remedy_quiz_modal,
          showCancelButton: false,
          showConfirmButton: false,
          customClass: {
            content: 'p-0 text-left'
          ...
          width: '30rem',
          onOpen: function onOpen() {
            $('.quiz-select22').append('<option selected="selected" value="' + remedy_quiz.id + '">' + remedy_quiz.title + '</option>');
            $(".quiz-select22").select2({
              placeholder: $(_this4).data('placeholder'),
              allowClear: true,
              width: '100%'
            });
            $('#addQuizModal').find('[name="chapter_id"]').val(remedy_quiz.chapter_id);
          }
        });
      }
    });
