...
window.handleWebinarItemForm = function (form, $this) {
  ...
  $.post(action, data, function (result) {
    ...
    if (errors && errors.errors) {
      Object.keys(errors.errors).forEach(function (key) {
        var error = errors.errors[key][0];
        // Find field by name (more reliable)
        var element = form.find('[name="' + key + '"]');

        if (element.length) {
          element.addClass('is-invalid');

          // If invalid-feedback div doesn’t exist, create it
          if (element.parent().find('.invalid-feedback').length === 0) {
            element.after('<div class="invalid-feedback d-block"></div>');
          }

          element.parent().find('.invalid-feedback').text(error);
        }
      });
    }
...
//   Products
   $('body').on('click', '#bundleAddNewProducts', function (e) {
    e.preventDefault();

    var html = '<div id="addBundleProductModal">';
    html += $('#bundleProductsModal').html();
    html += '</div>';
    html = html.replaceAll('bundleProducts-select', 'bundleProducts-select2');
    ...
    Swal.fire({
      ...
      width: '48rem',
      onOpen: function() {
        $('.bundleProducts-select2').select2({
          allowClear: true,
 ...
 $('body').on('click', '#saveBundleProduct', function (e) {
    e.preventDefault();
    var $this = $(this);
    var form = $('#addBundleProductModal .js-form');
    handleWebinarItemForm(form, $this);
  });
  $('body').on('click', '.edit-bundle-products', function (e) {
    e.preventDefault();
    var $this = $(this);
    var item_id = $this.attr('data-item-id');
    var product_id = $this.attr('data-bundle-id');
    loadingSwl();
    var edit_data = {
      item_id: product_id
    };
$.post(adminPanelPrefix + '/bundle-products/' + item_id + '/edit', edit_data, function (result) {
      if (result && result.bundleWebinar) {
        var bundleWebinar = result.bundleWebinar;
        var html = '<div id="addBundleProductModal">';
        html += $('#bundleProductsModal').html();
        html += '</div>';
        html = html.replaceAll('bundleProducts-select', 'bundleProducts-select2');
        html = html.replaceAll(adminPanelPrefix + '/bundle-products/store', adminPanelPrefix + '/bundle-products/' + item_id + '/update');
        html = html.replaceAll('str_', '');
        Swal.fire({
          html: html,
          showCancelButton: false,
          ...
          width: '48rem',
          onOpen: function onOpen() {
            var select = $('.bundleProducts-select2');
            select.val(bundleWebinar.product_id);
            select.select2({
              allowClear: false
            });
          }
...
$(document).ready(function () {
   ...
   $('body').on('click', '#bundleAddNewConsultation', function (e) {
    e.preventDefault();

    var html = '<div id="addBundleConsultationModal">';
    html += $('#bundleConsultationModal').html();
    html += '</div>';
    html = html.replaceAll('bundleConsultation-select', 'bundleConsultation-select2');
    ...
    Swal.fire({
      ...
      width: '48rem',
      onOpen: function() {
        $('.bundleConsultation-select2').select2({
          allowClear: true,
 ...
 $('body').on('click', '#saveBundleConsultation', function (e) {
    e.preventDefault();
    var $this = $(this);
    var form = $('#addBundleConsultationModal .js-form');
    handleWebinarItemForm(form, $this);
  });
  $('body').on('click', '.edit-bundle-consultation', function (e) {
    e.preventDefault();
    var $this = $(this);
    var item_id = $this.attr('data-item-id');
    var consultation_id = $this.attr('data-bundle-id');
    loadingSwl();
    var edit_data = {
      item_id: consultation_id
    };
$.post(adminPanelPrefix + '/bundle-consultations/' + item_id + '/edit', edit_data, function (result) {
      if (result && result.bundleWebinar) {
        var bundleWebinar = result.bundleWebinar;
        var html = '<div id="addBundleConsultationModal">';
        html += $('#bundleConsultationModal').html();
        html += '</div>';
        html = html.replaceAll('bundleConsultant-selec', 'bundleConsultant-selec2');
        html = html.replaceAll(adminPanelPrefix + '/bundle-consultations/store', adminPanelPrefix + '/bundle-consultations/' + item_id + '/update');
        html = html.replaceAll('str_', '');
        Swal.fire({
          html: html,
          showCancelButton: false,
          ...
          width: '48rem',
          onOpen: function onOpen() {
            var select = $('.bundleConsultant-selec2');
            select.val(bundleWebinar.consultation_id);
            select.select2({
              allowClear: false
            });
          }
