...
(function ($) {
  "use strict"; // form serialize to Object
  ...
  $('#summernote').summernote({
    ...
    dialogsInBody: true,
    toolbar: [['style', ['style']], ['font', ['bold', 'underline', 'clear']], ['fontname', ['fontname']], ['color', ['color']], ['para', ['paragraph']], ['table', ['table']], ['insert', ['link', 'picture', 'video']], ['view', ['fullscreen', 'codeview']]]
  });
  ...
  $(document).ready(function () {
    ...
    function updateToDatabase(table, idString) {
      $.post(adminPanelPrefix + '/subscriptions/order-items', {
        table: table,
        items: idString
      }, function (result) {
...
/**
 * add Subscription FAQ
 * */
$('body').on('click', '#subscriptionAddFAQ', function (e) {
    e.preventDefault();
    
    // Button se subscription_id lo
    var subscriptionId = $(this).attr('data-subscription-id');
    
    var add_faq_modal = '<div id="addFAQsModal">';
    add_faq_modal += $('#subscriptionFaqModal').html();
    add_faq_modal += '</div>';
    ...
    Swal.fire({
      ...
      width: '48rem',
      didOpen: function() {
        // Modal open hone ke baad subscription_id set karo
        var modal = $('#addFAQsModal');
        modal.find('input[name="subscription_id"]').val(subscriptionId);
...
$('body').on('click', '#saveSubscriptionFAQ', function (e) {
    e.preventDefault();
    var $this = $(this);
...
// Edit Subscription FAQ
function editSubscriptionFaq($this, locale) {
    ...
    $.post(adminPanelPrefix + '/faqs/' + faq_id + '/edit', edit_data, function (result) {
      if (result && result.faq) {
        ...
        edit_faq_modal += $('#subscriptionFaqModal').html();
        edit_faq_modal += '</div>';
        ...
        Swal.fire({
          ...
          didOpen: function() {
            ...
            if (localeSelect) {
              localeSelect.addClass('js-edit-subscription-faq-locale-ajax');
              localeSelect.attr('data-faq-id', faq_id);
              localeSelect.attr('data-subscription-id', subscription_id);
            }
          }
        });
      }
...
$('body').on('click', '.edit-subscription-faq', function (e) {
    e.preventDefault();
    var $this = $(this);
    loadingSwl();
    editSubscriptionFaq($this);
});

$('body').on('change', '.js-edit-subscription-faq-locale-ajax', function (e) {
    e.preventDefault();
    var $this = $(this);
    var locale = $this.val();
    editSubscriptionFaq($this, locale);
});
  ...
  * */
  ...
  function handleExtraInputHtml(key) {
    ...
    var modalHtml = $(html);
    // modalHtml.find('.js-form-groups').children().remove();
    modalHtml.find('.js-form-groups').append('<div class="form-group">\n' + '            <label class="input-label">image</label>\n' + '            <div class="input-group">\n' + '                <div class="input-group-prepend">\n' + '                    <button type="button" class="input-group-text admin-file-manager" data-input="image_' + key + '" data-preview="holder">\n' + '                        <i class="fa fa-upload"></i>\n' + '                    </button>\n' + '                </div>\n' + '                <input type="text" name="img" id="image_' + key + '" class="form-control"/>\n' + '            </div>\n' + '        </div>');
    var mainHtml = '<div id="extraDescriptionModal">';
    mainHtml += modalHtml.html();
    mainHtml += '</div>';
    return mainHtml;
  ...
  function editExtraDescription($this, locale) {
    ...
    $.post(adminPanelPrefix + '/webinar-extra-description/' + item_id + '/edit', edit_data, function (result) {
      if (result && result.webinarExtraDescription) {
        ...
        html += $('#extraDescriptionForm').html();
        html += '</div>';
 html = handleExtraInputHtml(rendomKey);
      ...
      if (result && result.prerequisite) {
        ...
        Swal.fire({
          ...
          onOpen: function onOpen() {
            ...
            $('.prerequisites-select2').select2({
              placeholder: $(_this3).data('placeholder'),
              minimumInputLength: 3,
              allowClear: true,
              ajax: {
                url: adminPanelPrefix + '/webinars/search',
      ...
      if (result && result.webinarQuiz) {
        ...
        Swal.fire({
          ...
          onOpen: function onOpen() {
            ...
            $(".quiz-select22").select2({
              placeholder: $(_this4).data('placeholder'),
              allowClear: true,
              width: '100%'
            });
...
//   Course
  $('body').on('click', '#subscriptionAddNewCourses', function (e) {
    e.preventDefault();
    var html = '<div id="addSubscriptionWebinarModal">';
    html += $('#subscriptionWebinarsModal').html();
    html += '</div>';
    html = html.replaceAll('subscriptionWebinars-select', 'subscriptionWebinars-select2');
    ...
    Swal.fire({
      ...
      width: '48rem',
      onOpen: function onOpen() {
        $('.subscriptionWebinars-select2').select2({
          allowClear: false
        });
      }
    });
  ...
  $('body').on('click', '#saveSubscriptionWebinar', function (e) {
    e.preventDefault();
    var $this = $(this);
    var form = $('#addSubscriptionWebinarModal .js-form');
    handleWebinarItemForm(form, $this);
  });
  $('body').on('click', '.edit-subscription-webinar', function (e) {
    e.preventDefault();
    var $this = $(this);
    var item_id = $this.attr('data-item-id');
    var subscription_id = $this.attr('data-subscription-id');
    ...
    $.post(adminPanelPrefix + '/subscription-webinars/' + item_id + '/edit', edit_data, function (result) {
      if (result && result.subscriptionWebinar) {
        var subscriptionWebinar = result.subscriptionWebinar;
        var html = '<div id="addSubscriptionWebinarModal">';
        html += $('#subscriptionWebinarsModal').html();
        html += '</div>';
        html = html.replaceAll('subscriptionWebinars-select', 'subscriptionWebinars-select2');
        html = html.replaceAll(adminPanelPrefix + '/subscription-webinars/store', adminPanelPrefix + '/subscription-webinars/' + item_id + '/update');
        html = html.replaceAll('str_', '');
        Swal.fire({
          html: html,
          showCancelButton: false,
          ...
          width: '48rem',
          onOpen: function onOpen() {
            var select = $('.subscriptionWebinars-select2');
            select.val(subscriptionWebinar.webinar_id);
            select.select2({
              allowClear: false
            });
          }
...
//   Products
   $('body').on('click', '#subscriptionAddNewProducts', function (e) {
    e.preventDefault();

    var html = '<div id="addSubscriptionProductModal">';
    html += $('#subscriptionProductsModal').html();
    html += '</div>';
    html = html.replaceAll('subscriptionProducts-select', 'subscriptionProducts-select2');
    ...
    Swal.fire({
      ...
      width: '48rem',
      onOpen: function() {
        $('.subscriptionProducts-select2').select2({
          allowClear: true,
 ...
 $('body').on('click', '#saveSubscriptionProduct', function (e) {
    e.preventDefault();
    var $this = $(this);
    var form = $('#addSubscriptionProductModal .js-form');
    handleWebinarItemForm(form, $this);
  });
  $('body').on('click', '.edit-subscription-products', function (e) {
    e.preventDefault();
    var $this = $(this);
    var item_id = $this.attr('data-item-id');
    var product_id = $this.attr('data-subscription-id');
    loadingSwl();
    var edit_data = {
      item_id: product_id
    };
$.post(adminPanelPrefix + '/subscription-products/' + item_id + '/edit', edit_data, function (result) {
      if (result && result.subscriptionWebinar) {
        var subscriptionWebinar = result.subscriptionWebinar;
        var html = '<div id="addSubscriptionProductModal">';
        html += $('#subscriptionProductsModal').html();
        html += '</div>';
        html = html.replaceAll('subscriptionProducts-select', 'subscriptionProducts-select2');
        html = html.replaceAll(adminPanelPrefix + '/subscription-products/store', adminPanelPrefix + '/subscription-products/' + item_id + '/update');
        html = html.replaceAll('str_', '');
        Swal.fire({
          html: html,
          showCancelButton: false,
          ...
          width: '48rem',
          onOpen: function onOpen() {
            var select = $('.subscriptionProducts-select2');
            select.val(subscriptionWebinar.product_id);
            select.select2({
              allowClear: false
            });
          }
...
$(document).on('change', '#consultationType', function () {
    let value = $(this).val();

 
    $("#specificConsultant, #priceRange, #slotTime").addClass('d-none');

    if (value === "specific") {
        $("#specificConsultant, #slotTime").removeClass('d-none');
    } 
    else if (value === "range") {
        $("#priceRange, #slotTime").removeClass('d-none');
    } 
    else if (value === "all") {
       $("#slotTime, #slotTime_modal").removeClass('d-none');
    }
...
$(document).ready(function () {
    $('#consultationType').trigger('change');
});


   $('body').on('click', '#subscriptionAddNewConsultation', function (e) {
    e.preventDefault();

    var html = '<div id="addSubscriptionConsultationModal">';
    html += $('#subscriptionConsultationModal').html();
    html += '</div>';
    html = html.replaceAll('subscriptionConsultation-select', 'subscriptionConsultation-select2');
    ...
    Swal.fire({
      ...
      width: '48rem',
      onOpen: function() {
        $('.subscriptionConsultation-select2').select2({
          allowClear: true,
 ...
 $('body').on('click', '#saveSubscriptionConsultation', function (e) {
    e.preventDefault();
    var $this = $(this);
    var form = $('#addSubscriptionConsultationModal .js-form');
    handleWebinarItemForm(form, $this);
  });
  $('body').on('click', '.edit-subscription-consultation', function (e) {
    e.preventDefault();
    var $this = $(this);
    var item_id = $this.attr('data-item-id');
    var consultation_id = $this.attr('data-subscription-id');
    loadingSwl();
    var edit_data = {
      item_id: consultation_id
    };
$.post(adminPanelPrefix + '/subscription-consultations/' + item_id + '/edit', edit_data, function (result) {
      if (result && result.subscriptionWebinar) {
        var subscriptionWebinar = result.subscriptionWebinar;
        var html = '<div id="addSubscriptionConsultationModal">';
        html += $('#subscriptionConsultationModal').html();
        html += '</div>';
        html = html.replaceAll('subscriptionConsultant-selec', 'subscriptionConsultant-selec2');
        html = html.replaceAll(adminPanelPrefix + '/subscription-consultations/store', adminPanelPrefix + '/subscription-consultations/' + item_id + '/update');
        html = html.replaceAll('str_', '');
        Swal.fire({
          html: html,
          showCancelButton: false,
          ...
          width: '48rem',
          onOpen: function onOpen() {
            var select = $('.subscriptionConsultant-selec2');
            select.val(subscriptionWebinar.consultation_id);
            select.select2({
              allowClear: false
            });
          }
