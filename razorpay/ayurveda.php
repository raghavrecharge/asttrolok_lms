...
<input type="hidden" name="merchant_order_id" id="merchant_order_id" value="ayurveda"> 
...
<input type="hidden" name="furl" id="furl" value="https://www.asttrolok.com/razorpay/failed.php"> 
       ...
       <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" >
		...
		?>
		<div id="">
			<label><a id="coupenlink" href="javascript:void(0)">Do You have coupon ? <span style="color:blue;">Please click here.</span></a></label>
			<input type="text"  id="coupen" class="require" name="coupen" placeholder="coupon code" value="">
		</div>
		   
	   		<label for="inputEmail4">Amount</label>
		   <input type="number"  id="amount" class="require coupenamount" name="amount" placeholder="amount" value="<?php echo $originalPrice; ?>" disabled>
		<?php //} else { ?>
		   <!--
		   <label for="inputEmail4">Amount</label>
		   <input type="number"  id="amount" class="require" name="amount" placeholder="amount" value="">
			-->
		<?php 
			//}
		   ?>
...
<script type="text/javascript">
  jQuery(document).on('click', '#razor-pay-now', function (e) {
    ...
    var store_description = 'Ayurveda Course Payment';
...
$(document).ready(function(){
  $("#coupen").hide();
	...
	$("#coupenlink").click(function(){
		$("#coupen").show();
	});
	
	$("#coupen").change(function(){
		var code = "Asttroveda123";
		var coupen = $("#coupen").val();
		var originalPrice = 2999;
		var discount = (originalPrice/100)*10;
		var discountedPrice = originalPrice - discount;
		
		if(coupen === code){
		   $('.coupenamount').val(Math.floor(discountedPrice));
		} else{
			$('.coupenamount').val(originalPrice);
		}
