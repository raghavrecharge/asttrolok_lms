...
if (!function_exists('fetchNavbarCategories')) {
    function fetchNavbarCategories()
    {
...
function dateTimeFormat($timestamp, $format = 'H:i', $useAdminSetting = true, $applyTimezone = true, $timezone = "UTC")
{
    ...
    if ($timestamp instanceof Carbon\Carbon) {
        $carbon = $timestamp->copy()->setTimezone($timezone);
        ...
        // Handle string dates
        $carbon = Carbon\Carbon::parse($timestamp)->setTimezone($timezone);
    }
...
function setCurrencyByCountry()
    {
        ...
        $currencyMap = [
            ...
            'AQ' => 'No universal currency', // Antarctica - No universal currency
            ...
            'GA' => 'GAB', // Gabon - Central African CFA Franc
            'GM' => 'GMB', // Gambia - Gambian Dalasi
            'GE' => 'GEO', // Georgia - Georgian Lari
            'DE' => 'EUR', // Germany - Euro
            'GH' => 'GHS', // Ghana - Ghanaian Cedi
            'GI' => 'GIP', // Gibraltar - Gibraltar Pound
            'GR' => 'GRD', // Greece - Greek Drachma
            'GL' => 'DKK', // Greenland - Danish Krone
            'GD' => 'GRD', // Grenada - Grenadian Dollar
            'GP' => 'EUR', // Guadeloupe - Euro
            'GU' => 'USD', // Guam - US Dollar
            'GT' => 'GTQ', // Guatemala - Guatemalan Quetzal
            'GG' => 'GBP', // Guernsey - Guernsey Pound
            'GN' => 'GNF', // Guinea - Guinean Franc
            'GW' => 'GNF', // Guinea-Bissau - Guinean CFA Franc
            ...
            'IS' => 'ISK', // Iceland - Icelandic Kr贸na
            ...
            'IT' => 'EUR', // Italy - Euro
            'CI' => 'CIV', // Ivory Coast - CFA Franc BCEAO
            'JM' => 'JMD', // Jamaica - Jamaican Dollar
            'JP' => 'JPY', // Japan - Japanese Yen
            'JE' => 'GBP', // Jersey - Jersey Pound
            ...
            'LU' => 'LUF', // Luxembourg - Luxembourg Franc
            ...
            'US' => 'USD', // Micronesia - US Dollar

        ];
        
        
  
        $currencyq = $currencyMap[$countryCode] ?? 'USD';
        $multiCurrency = new MultiCurrency();
        $currencies = $multiCurrency->getCurrencies();
        $available = $currencies->pluck('currency')->toArray();
         $exists = $currencies->contains(function ($currency) use ($currencyq) {
            return $currency->currency === $currencyq;
        });
        
        if ($exists) {
        $userCurrency1 = $currencyq;
        } else {
        $userCurrency1 ="USD";
        }
    //   echo $countryCode;
      if($countryCode !="IN"){
        ...
        if(!Session::get('selectbysuer')){
            if (in_array($userCurrency1, $available)) {
            if (auth()->check()) {
                $user = auth()->user();
                $user->update([
                    'currency' => $userCurrency1
                ]);
            } else {
                Cookie::queue('user_currency', $userCurrency1, 30 * 24 * 60);
            } 
        }else{
            Session::put('selectbysuer',false);
        }

         
        }
        return $userCurrency1;
...
if (!function_exists('getOptimizedImage')) {
    function getOptimizedImage($imagePath, $checkWebP = true)
    {
        ...
        $relativePath = str_replace($imgUrl, '', $imagePath);

        // if ($checkWebP && isset($_SERVER['HTTP_ACCEPT']) && strpos($_SERVER['HTTP_ACCEPT'], 'image/webp') !== false) {
        //     $webpPath = $relativePath;

        //     if (Storage::disk('gcs')->exists($webpPath)) {
        //         return $imgUrl . $webpPath;
        //     }
        // }
...
if (!function_exists('getOptimizedImagewebp')) {
    function getOptimizedImagewebp($imagePath, $checkWebP = true)
    {
        $imgUrl = config('app.img_dynamic_url'); 
        ...
        $optimizedPath = str_replace('/store/', '/store2/store2/', $imagePath);

        if ($checkWebP && isset($_SERVER['HTTP_ACCEPT']) && strpos($_SERVER['HTTP_ACCEPT'], 'image/webp') !== false) {
            $webpPath = preg_replace('/\.(jpg|jpeg|png)$/i', '.webp', $optimizedPath);
        ...
        return rtrim($imgUrl, '/') . $optimizedPath;
    }
}
