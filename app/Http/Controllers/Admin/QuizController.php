...
class QuizController extends Controller
{
    ...
    public function copy(Request $request, $id)
    {
        ...
        $quizCopy = Quiz::create([
                ...
                'display_limited_questions' => $quiz->display_limited_questions,
                'display_number_of_questions' => $quiz->display_number_of_questions,
                'status' => 'inactive',
            ...
            QuizTranslation::Create([
                'quiz_id' => $quizCopy->id,
                'locale' => mb_strtolower($locale),
                'title' => $quiz->getTitleAttribute().'-copy',
            ]);
        
        if (!empty($quiz)) {
            foreach($quiz->quizQuestions as $quizQuestion){
                $creator = $quiz->creator;
                $order = QuizzesQuestion::query()->where('quiz_id', $quizCopy->id)->count() + 1;
                
                $quizQuestionCopy = QuizzesQuestion::create([
                    'quiz_id' => $quizCopy->id,
                    'creator_id' => $creator->id,
                    'grade' => $quizQuestion->grade,
                    'type' => $quizQuestion->type,
                    'image' => $quizQuestion->image ?? null,
                    'video' => $quizQuestion->video ?? null,
                    'order' => $order,
                    'created_at' => time()
                ]);
                if (!empty($quizQuestionCopy) and !empty($quizQuestion)) {
                    QuizzesQuestionTranslation::Create([
                        'quizzes_question_id' => $quizQuestionCopy->id,
                        'locale' => mb_strtolower($locale),
                        'title'=> $quizQuestion->title ?? null,
                        'correct' => $quizQuestion->correct ?? null,
                    ]);
                }
                
                $answers = $quizQuestion->quizzesQuestionsAnswers;
                // $quizCopy->increaseTotalMark($quizQuestion->grade);
                    ...
                    foreach ($answers as $answer) {
                        if (!empty($answer->title) or !empty($answer->file)) {
                            $questionAnswer = QuizzesQuestionsAnswer::create([
                                'question_id' => $quizQuestionCopy->id,
                                'creator_id' => $creator->id,
                            ...
                            $QuizzesQuestionsAnswerTranslation = QuizzesQuestionsAnswerTranslation:: where('quizzes_questions_answer_id',$answer->id)->first();
                            if (!empty($questionAnswer)) {
                                $QuizzesQuestionsAnswerTranslationCopy = QuizzesQuestionsAnswerTranslation::Create([
                                    'quizzes_questions_answer_id' => $questionAnswer->id,
                                    'locale' => mb_strtolower($locale),
                                    'title' => $QuizzesQuestionsAnswerTranslation->title,
                                ]);
                            }
                        }
                    }
        ...
        return redirect('/admin/quizzes/'.$quizCopy->id.'/edit');
    }
