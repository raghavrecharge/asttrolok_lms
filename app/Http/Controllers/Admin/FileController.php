...
$sourceDefaultFileTypeAndVolume1 = ['secure_host_link'];
        if (in_array($data['storage'], $sourceDefaultFileTypeAndVolume)) {
            $data['file_type'] = 'video';
            $data['volume'] = 0;
        }
if (in_array($data['storage'], $sourceDefaultFileTypeAndVolume1)) {
            $data['file_type'] = 'video';
            $data['volume'] = 0;
        }
        $rules = [
        ...
        if (!empty($webinar)) {
            ...
            if (!empty($file)) {
                ...
                $webid=$data['webinar_id'];
                $webinar = Webinar::where('id', $webid)
            ->with([
            ...
            ->first();
  $webinarTranslationwebinar = WebinarTranslation::where('webinar_id', $webid)
            ...
            foreach ($webinar->sales as $sale) {
                ...
                if (!empty($sale->buyer)) {
                    ...
                    Notification::create([
                        ...
                        'message' => 'New Chapter '.$datatitle.' Added',
 ...
 $contactData = [
            'email' => $user->email,
            'fields' => [
                '934416' => $webinarTranslationwebinar->title,
               '456805' => $user->full_name,
                // Add other contact fields as needed
            ],
        ];
        ...
        // echo "<pre>";print_r($result);die;
                    ...
                    //     \Mail::to($user->email)->send(new SendNotifications(['title' => 'New Chapter Added', 'message' => 'New Chapter '.$datatitle.'  Added']));
                    // }
                }
            }
        }
