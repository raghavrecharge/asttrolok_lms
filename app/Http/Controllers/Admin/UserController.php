...
if(empty($data['consultant'])){
    ...
    {
        // $CourseProgress = CourseProgress :: where('user_id',$id)
        //     ->where('slug',$slug)
        //     ->get();
            
        // echo "user id ".$id."<br>";
        // echo "Webinar id ".$slug."<br><pre>";
...
// print_r($data['course']);die();
    ...
    public function editinsta(Request $request, $id , $instaid)
    {
        ...
        if (!empty($instaid)) {
            ...
            if ($installmentClasses[0]->installment->type != 'step') {
                ...
                // DB::update('update installment_orders set status = ? where id = ?',[$status,$id2]);
                DB::delete('delete from installment_orders where id  = ?',[$id2]);
                // print_r($installmentClasses[0]->installment->type);
            }
        ...
        // print_r($installmentClasses[0]->installment->installmentOrder->id);
        }
    ...
    private function getPurchasedInstaClassesData($user)
    {
        ...
        $installmentClasses = Sale::whereNull('refund_at')
            ...
            ->get();
        //     $installmentClasses1=' ';
        //   foreach($installmentClasses as $installmentClasse){
              
        //  $installmentClasses1 +='~'. $installmentClasse->installment->installmentorder->webinar->id;  
        //  $count++;
        //   }

        return [
            'installmentClasses' => $installmentClasses,
            // 'installmentClasses1' => $installmentClasses1,
        ];
    }
