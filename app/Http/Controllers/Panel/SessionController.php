...
class SessionController extends Controller
{
    ...
    public function joinToBigBlueButton($id)
    {
        try {
            ...
            \Log::error('joinToBigBlueButton error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function joinToAgora($id)
    {
        try {
            ...
            \Log::error('joinToAgora error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function endAgora($id)
    {
        try {
            ...
            \Log::error('endAgora error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
        ...
        try {
            ...
            if (!empty($session)) {
                ...
                if ($session->creator_id == $user->id or (!empty($webinar) and $webinar->canAccess($user))) {
                    ...
                    $userJoin = (!empty($sessionSettings) and !empty($sessionSettings->users_join) and $sessionSettings->users_join) ? false : true;
            ...
            \Log::error('toggleUsersJoinToAgora error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function joinToJitsi($id)
    {
        try {
            ...
            \Log::error('joinToJitsi error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
