    ...
    public function index()
    {
        ...
        if (!empty($verificationId)) {
            ...
            if (!empty($verification)) {
                ...
                $agent = new Agent();
                                    if ($agent->isMobile()){
                                        return view(getTemplate() . '.auth.verification', $data);
                                }else{
                                    return view('web.default2' . '.auth.verification', $data);
                                }
...
// print_r($verification->sendEmailCode());die();
        ...
        if (empty($authUser)) {
            ...
            if (!empty($authUser)) {
                if (\Auth::loginUsingId($authUser->id)) {
                    ...
                    // $WebinarController->free($request, 'Free-Astrology-Course');
                    // session(['freecourse' => 'Free-Astrology-Course']);
