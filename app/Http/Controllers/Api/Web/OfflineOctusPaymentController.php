    ...
    private function handlePayStep($order, $step)
     {
        ...
        $paidAmount = null;
    // print_r($order);die;
        // Fetch total part payment made for this webinar by user
        ...
        if ($WebinarPartPayment) {
            ...
            // Get total amount already paid in successful steps
        ...
        // Cart::where('creator_id', $user->id)->delete();
...
// print_r($total);die();
     ...
     public function installmentBackgroundProcess($request)
    {
       ...
       if (isset($amount)) {
    // If user is paying by part payment link
    if ($amount >= $itemPrice) {
        
        // print_r($itemPrice);die;
        // Paid full payment for the course
        ...
        // Get the discount coupon if valid
        $discountCoupon = Discount::where('id', $discountId)->first();
        ...
        // Payment verification and post-processing
        ...
        // User has not paid the full amount (this else block is likely continued)
    ...
    if ($part_amount_status) {
        ...
        // Update part amount in sale, order and order item
        $sale = Sale::where('buyer_id', $user->id)
            ->where('webinar_id', $itemId)
            ->first();
    ...
    if (!empty($installment)) {
        ...
        if (!empty($item)) {
            ...
            // Already existing sale, update total_amount and related records
            $sale = Sale::where('buyer_id', $user->id)
                ->where('webinar_id', $itemId)
                ->first();
    ...
    // public function fullAccessByoctuss(){
    ...
    public function fullAccessByoctus(Request $request)
        {
            ...
            if ($installments->isNotEmpty()) {
                ...
                // handle the case when no installment is found
                return response()->json([
                    'success' => false,
