...
//         // Step 1: Get active forums where user has access
//         $forums = Forum::where('status', 'active')
//             ->get()
//             ->filter(function ($forum) {
//                 return $forum->checkUserCanCreateTopic(); // or any access logic
//             });

//         $responseData = [];

//         foreach ($forums as $forum) {
//             // Step 2: Get topics under each forum
...
//         return apiResponse2(1, 'retrieved', trans('api.public.retrieved'), $responseData);
//     }
...
//         // Step 1: Get all post records where this user has posted
//         $userPosts = ForumTopicPost::where('user_id', $user->id)
//             ->with(['topic.forum']) // eager load topic and forum
//             ->get();
...
//             // Skip if topic or forum is missing or inactive
//             if (!$topic || !$topic->forum || $topic->forum->status !== 'active') {
//                 continue;
//             }
...
//             if (!isset($groupedTopics[$topicId])) {
//                 // Get all posts of the topic
//                 $topicWithPosts = ForumTopic::where('id', $topicId)
//                     ->with([
//                         'forum',
//                         'attachments',
//                         'posts' => function ($query) {
...
//                 // Add user info for topic creator
//                 $Userpost = User::select("id", 'full_name', 'avatar')
//                     ->where('id', $topicWithPosts->creator_id)
//                     ->first();
...
//                 $topicWithPosts->liked = $topicLiked;
//                 $topicWithPosts->bookmarked = $bookmarked;

//                 // Add user info to each post
//                 foreach ($topicWithPosts->posts as $postItem) {
//                     $postUser = User::select('full_name', 'avatar')
//                         ->where('id', $postItem->user_id)
//                         ->first();

//                     $postItem->user = $postUser;
//                 }

//                 // Store in grouped array
//                 $groupedTopics[$topicId] = [
//                     'forum' => $topicWithPosts->forum,
...
//         // Final response
//         return apiResponse2(1, 'retrieved', trans('api.public.retrieved'), array_values($groupedTopics));
//     }
