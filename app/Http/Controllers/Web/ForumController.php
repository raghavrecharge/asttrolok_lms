 ...
 public function tutorialGuide()
    {
        try {
            ...
            \Log::error('tutorialGuide error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function createTopic(Request $request ,$error125=null)
    {
        try {
            $user = auth()->user();
            ...
            if(!empty($error125)){
             ...
             $data = ['pageTitle' => trans('update.create_new_topic'),
                'pageDescription' => '',
                'pageRobot' => '',
                'forums' => $forums,
                'error125' => $error125];
            ...
            if (!empty($forum) and $forum->checkUserCanCreateTopic($user)) {
               ...
               $forum_topic = ForumTopic::where('slug', $slug1)
                ...
                return redirect($url)->with(['toast' => $toastData]);
                  ...
                  return  $this->createTopic($request,$error125);

                }

            }

            return redirect('/panel/forums/topics');
        } catch (\Exception $e) {
            \Log::error('storeTopic error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function topicLikeToggle($forumSlug, $topicSlug)
    {
        try {
            ...
            \Log::error('topicLikeToggle error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function topicBookmarkToggle($forumSlug, $topicSlug)
    {
        try {
            ...
            \Log::error('topicBookmarkToggle error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function topicEdit($forumSlug, $topicSlug)
    {
        try {
            ...
            \Log::error('topicEdit error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function topicUpdate(Request $request, $forumSlug, $topicSlug)
    {
        try {
            ...
            \Log::error('topicUpdate error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function topicDownloadAttachment($forumSlug, $topicSlug, $attachmentId)
    {
        try {
            ...
            \Log::error('topicDownloadAttachment error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function storeTopicReport(Request $request, $forumSlug, $topicSlug)
    {
        try {
            ...
            \Log::error('storeTopicReport error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function postLikeToggle($forumSlug, $topicSlug, $postId)
    {
        try {
            ...
            \Log::error('postLikeToggle error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function postDownloadAttachment($forumSlug, $topicSlug, $postId)
    {
        try {
            ...
            \Log::error('postDownloadAttachment error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
