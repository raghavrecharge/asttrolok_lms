...
//         }
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            //echo '<pre>'; print_r($data);die;
                // return view('web.default.cart.cart1', $data);
            }
...
// print_r('mayank');
     ...
     public function couponValidate3(Request $request)
    {
        ...
        //   }
            ...
            // print_r($discountCoupon->id);
            // print_r($discountCoupon->source);
            // print_r($discountCoupon->percent);
            // print_r($discountCoupon->status);
            ...
            // die();
                ...
                // if (!empty($calculate)) {
                    ...
                    // return redirect('/users/'.$user_id.'/profile?tab=appointments');
                    return response()->json([
                        'status' => 200,
...
// echo $discountCoupon;
        ...
        //             'item_id' => $web_id1
        //             ], 200);
        // ($webinarController->directPayment($request));
                    return $webinarController->directPayment($request);
        return $coupon;
        }else{
            if($coupon)
    ...
    {
        // Store only the values that are sent in the request
        $sessionData = [];
    ...
    public function couponValidate(Request $request)
    {
        ...
        if (!empty($discountCoupon)) {
           ...
           if(session('cart_id')){
            ...
            //      $carts1 = Cookie::get('carts');
            //      $carts = json_decode($carts1,true);
            //     $checkDiscount = $discountCoupon->checkValidDiscount2();
            // if ($checkDiscount != 'ok') {
            //     return response()->json([
            //         'status' => 422,
            //         'msg' => $checkDiscount
            ...
            //     $calculate = $this->calculatePrice1($carts, $user, $discountCoupon);
            //     if (!empty($calculate)) {
            //         session(['discount_id' => $discountCoupon->id]);
            //         session(['total_discount' => handlePrice($calculate["total_discount"])]);
            //         session(['total_tax' => handlePrice($calculate["tax_price"])]);
...
// print_r($total);die();
    ...
    //   print_r($discountCoupon); die();
        // $checkv=$discountCoupon->checkValidDiscount();
        // echo $checkv;
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            $order = $this->createOrderAndOrderItems($carts, $calculate, $user, $discountCoupon);
//   echo $discountCoupon;die('mayank');
            if (!empty($discountCoupon)) {
                ...
                foreach ($cart->bundle->bundleWebinars as $bundleWebinar){
                    if($bundleWebinar->product_id){
                        ...
                        if (!empty($product) and !empty($productsFee[$product->creator_id])) {
                            ...
                            // }else{
                            //     $productDeliveryFee = $product->delivery_fee_international;
                            // }
