...
class Subscription extends Model implements TranslatableContract
{
    ...
    {
        return $this->hasMany('App\Models\SubscriptionFilterOption', 'subscription_id', 'id');
    }

    public function tags()
    {
        return $this->hasMany('App\Models\Tag', 'subscription_id', 'id');
    }

    public function tickets()
    {
        return $this->hasMany('App\Models\Ticket', 'subscription_id', 'id');
    }

    public function subscriptionWebinars()
    {
        return $this->hasMany('App\Models\SubscriptionWebinar', 'subscription_id', 'id');
    }

    public function faqs()
    {
        return $this->hasMany('App\Models\Faq', 'subscription_id', 'id');
    }
...
public function whyChooseUsItems()
{
    ...
    {
        return $this->hasMany('App\Models\Comment', 'subscription_id', 'id');
    }

    public function reviews()
    {
        return $this->hasMany('App\Models\WebinarReview', 'subscription_id', 'id');
    }

    public function sales()
    {
        return $this->hasMany('App\Models\Sale', 'subscription_id', 'id')
            ->whereNull('refund_at')
    ...
    {
        if (empty($this->subscriptionDuration)) {
            $this->subscriptionDuration = $this->newQuery()
                ->where('subscriptions.id', $this->id)
                ->join('subscription_webinars', 'subscription_webinars.subscription_id', 'subscriptions.id')
                ->join('webinars', 'webinars.id', 'subscription_webinars.webinar_id')
                ->select('subscriptions.*', DB::raw('sum(webinars.duration) as duration'))
                ->sum('duration');
        }
    ...
    {
        $activeSpecialOffer = SpecialOffer::where('subscription_id', $this->id)
...
public function getProgress($isLearningPage = false)
    ...
    {
        $studentsIds = Sale::where('subscription_id', $this->id)
            ->whereNull('refund_at')
            ->pluck('buyer_id')
            ->toArray();
...
{
    return $this->hasOne(SubscriptionExtraDetails::class, 'subscription_id');
}
