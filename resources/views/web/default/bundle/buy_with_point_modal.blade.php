<div class="d-none" id="buyWithPointModal">
    ...
    @if(!empty($user))
        <div class="text-center">
            <img loading="lazy" src="{{ config('app.js_css_url') }}/assets/default/img/rewards/medal-2.png" class="buy-with-points-modal-img" alt="medal">

            <p class="font-14 font-weight-500 text-gray mt-30">
                <span class="d-block">{{ trans('update.this_course_requires_n_points',['points' => $bundle->points]) }}</span>
        ...
        <div class="d-flex align-items-center mt-25">
            <a href="{{ ($user->getRewardPoints() >= $bundle->points) ? '/bundles/'. $bundle->slug .'/points/apply' : '#' }}" class="btn btn-sm flex-grow-1 {{ ($user->getRewardPoints() >= $bundle->points) ? 'btn-primary js-buy-course-with-point' : 'bg-gray300 text-gray disabled' }}">{{ trans('update.buy') }}</a>
