                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        ...
                                        $statusClass = 'badge-' . ($statusColors[$req->status] ?? 'secondary');
                                ...
                                <td>
                                    @if($req->status === 'rejected' && $req->rejected_reason)
                                        <span class="text-danger font-12" title="{{ $req->rejected_reason }}">
                                            <i data-feather="info" width="14" height="14"></i> {{ \Illuminate\Support\Str::limit($req->rejected_reason, 30) }}
                                        </span>
                                    @elseif($req->status === 'executed' && $req->execution_result)
                                        <span class="text-primary font-12"><i data-feather="check" width="14" height="14"></i> Done</span>
                                    @elseif($req->payload)
                                        @if(!empty($req->payload['amount']))
                                            <span class="font-12">{{ handlePrice($req->payload['amount']) }}</span>
                                        @endif
