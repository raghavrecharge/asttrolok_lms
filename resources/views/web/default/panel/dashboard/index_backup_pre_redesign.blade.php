...
<div class="userprofile d-flex align-items-center">
        ...
        <!-- $extended = $extendedAccesses[$item->id][0] ?? null; -->
  @php
    $extendedExpire = $extendedAccesses[$item->id] ?? null;
@endphp

@if(!empty($extendedExpire))
    <span class="badge badge-outlined-warning">
        Extended till {{ \Carbon\Carbon::parse($extendedExpire)->format('j M Y') }}
    </span>
@else
                                                    @if(!$item->checkHasExpiredAccessDays((isset($item->start_date) ?$item->start_date:$sale->created_at), $sale->gift_id))
                                                        <span class="badge badge-outlined-danger ">{{ trans('update.access_days_expired') }}</span>
