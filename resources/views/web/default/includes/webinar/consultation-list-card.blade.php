<div class="webinar-card list-card webinar-list webinar-list-2 d-flex mt-20 loadmorelist">
    ...
    <div class="webinar-card-body w-100 d-flex flex-column">
       ...
       {{-- @if(!empty($consultation->category))
            <span class="d-block font-14 mt-10 hide">{{ trans('public.in') }} <a href="{{ $consultation->category->getUrl() }}" target="_blank" class="text-decoration-underline">{{ $consultation->category->title }}</a></span>
        @endif --}}
        ...
        @if($consultation->consultation_type == 'specific' && $consultation->consultant_id)
           ...
           {{-- @include(getTemplate() . '.includes.webinar.rate3',['rate' => $consultation->getRate()]) --}}
