<div class="radius-20 stars-card d-flex align-items-center adding-left: 1px; padding-right:1px; padding-top:1px;">
    @php
        $i = 5;
    @endphp
    ...
    @if((!empty($rate) and $rate > 0) or !empty($showRateStars))
        ...
        @endwhile
        @while($i-- >= 0)
            <i data-feather="star" width="14" height="14" class="" style="padding-left:1px;"></i>
        @endwhile
        @if(empty($dontShowRate) or !$dontShowRate)
            <span class="radius-20  badge-primary1 shadow-sm ml-5" style="padding-bottom: 2px;
    padding-top: 2px;
    padding-left: 3px;
    padding-right: 4px;
    font-size: 11px;
