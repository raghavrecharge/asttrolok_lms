...
<div class="learning-content" id="learningPageContent">1
    ...
    @if(!empty($isForumAnswersPage) and $isForumAnswersPage)
        @include('web.default.subscriptionProgress.learningPage.components.forum.forum_answers')
    @elseif(!empty($isForumPage) and $isForumPage)
        @include('web.default.subscriptionProgress.learningPage.components.forum.forum')
    @elseif(!empty($noticeboards) and $noticeboards)
        @include('web.default.subscriptionProgress.learningPage.components.noticeboards')
    @elseif(!empty($assignment))
        @include('web.default.subscriptionProgress.learningPage.components.assignment')
    @endif
<div id="learningPageContent1">
    <div class="learning-content-loading align-items-center justify-content-center flex-column w-100 h-100 {{ $showLoading ? 'd-flex' : 'd-none' }}">
        <img loading="lazy" src="{{ config('app.js_css_url') }}/assets/default/img/loading.gif" alt="">
