...
<div class="learning-page-navbar d-flex align-items-lg-center justify-content-between flex-column flex-lg-row px-15 px-lg-35 py-10">
    <div class="d-flex align-items-lg-center flex-column flex-lg-row flex-grow-1">
        ...
        <div class="learning-page-logo-card d-flex align-items-center justify-content-between justify-content-lg-start">
            ...
            <div class="d-flex align-items-center d-lg-none ml-20">
                <a href="{{ $subscription->getUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200 d-none d-md-block">{{ trans('update.course_page') }}</a>
    ...
    <div class="d-flex align-items-center mt-5 mt-md-0">
        ...
        @if(!empty($subscription->noticeboards_count) and $subscription->noticeboards_count > 0)
            <a href="{{ $subscription->getNoticeboardsPageUrl() }}" target="_blank" class="btn learning-page-navbar-btn noticeboard-btn btn-sm border-gray200 mr-10">
                <i data-feather="bell" class="" width="16" height="16"></i>

                <span class="noticeboard-btn-badge d-flex align-items-center justify-content-center text-white bg-danger rounded-circle font-12">{{ $subscription->noticeboards_count }}</span>
            </a>
        @endif

        @if($subscription->forum)
            <a href="{{ $subscription->getForumPageUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200 mr-10">{{ trans('update.course_forum') }}</a>
        @endif

        <div class="d-none align-items-center d-lg-flex">
            <a href="{{ $subscription->getUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200">{{ trans('update.course_page') }}</a>
