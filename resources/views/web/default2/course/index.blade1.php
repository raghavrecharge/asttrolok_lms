                ...
                <div class="row">
                    <div class="col-12 col-lg-6 course-section-top">
                        ...
                        <iframe style="border-radius: 30px; cursor:pointer; text-align: center; width:100%; height:280px" src="{{ $course->video_demo_source == 'upload' ?  url($course->video_demo) : $course->video_demo }}" title="Asttrolok" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"  allowfullscreen=""></iframe>
                            ...
                            <!--</div>-->
                            @else
                              <img src="{{ config('app.img_dynamic_url') }}{{ $course->getImage() }}" class="img-cover" style="height:auto" alt="webinar Demo Video">
                        @endif
                        @if((isset($course->start_date) and $course->isCourse()))
                        <div style="background-color: white; padding: 10px; border: 1px solid #ddd; border-radius: 8px; text-align: center; max-width: 400px; margin: 20px auto;">
    
    <div id="course-timer" style="font-size: 20px; font-weight: bold; color: green;">
         Loading...
    </div>
    
    <div id="buy-now-form" style="display: none; margin-top: 20px;">
        <form action="/cart/store" method="post">
            <input type="hidden" name="_token" value="sU2zwrsBIElpKSvhnsArRF0YfJAjmcsTXKSGMJBu">
            <input type="hidden" name="item_id" value="2102">
            <input type="hidden" name="item_name" value="webinar_id">
            <button type="button" class="btn-primary btn buy_now mt-20 js-course-direct-payment">
                Buy now!
            </button>
        </form>
    </div>
                    ...
                    <div class="col-12 col-lg-6 text-center course-section-top">
                        ...
                        <img src="{{ config('app.img_dynamic_url') }}{{ $course->getImageCover() }}" class="img-cover" style="height:auto;" alt="webinar Demo Video">
...
//   $('.modal-dialog').addClass('afterpop');
    ...
    <script>
const courseStartDate = new Date('{{ \Carbon\Carbon::parse($course->start_date)->format('Y-m-d\TH:i:s') }}').getTime();
let now = new Date().getTime();

const timerInterval = setInterval(function() {
    const timeLeft = courseStartDate - now;
    ...
    if (timeLeft > 0) {
        ...
        document.getElementById('course-timer').innerText =
            ` ${days}d ${hours}h ${minutes}m ${seconds}s`;
        
        now += 1000;
    } else {
        clearInterval(timerInterval);
        // Timer khatam hone par "Buy Now" form dikhega
        document.getElementById('buy-now-form').style.display = 'block';
    }
}, 1000);
</script>
