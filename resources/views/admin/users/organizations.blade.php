...
<div class="um-page-container bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 antialiased h-full flex flex-col p-4 md:p-8">
    ...
    <div class="mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight">{{ trans('admin/main.organizations_list') }}</h2>
            <p class="text-slate-500 text-sm mt-1">Manage platform corporate partners and organizations ({{ $users->total() }} Total)</p>
        </div>
        ...
        <form method="get" id="umFilterForm" class="flex items-center gap-3 flex-wrap">
            ...
            <div class="flex items-center bg-white border border-slate-200 rounded-xl p-1 shadow-sm h-10">
                ...
                <select name="group_id" class="bg-transparent border-none text-sm font-medium text-slate-600 focus:ring-0 cursor-pointer px-3 h-full max-w-[150px]" onchange="this.form.submit()">
                    <option value="">{{ trans('admin/main.select_users_group') }}</option>
                    @foreach($userGroups as $userGroup)
                        <option value="{{ $userGroup->id }}" {{ request()->get('group_id') == $userGroup->id ? 'selected' : '' }}>{{ $userGroup->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <a href="{{ getAdminPanelUrl() }}/organizations/excel?{{ http_build_query(request()->all()) }}" class="bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-bold h-10 px-4 rounded-xl flex items-center gap-2 transition-all shadow-sm shrink-0 no-underline text-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-5 group hover:border-primary/30 transition-all cursor-default overflow-hidden">
            ...
            <div>
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ trans('admin/main.total_organizations') }}</p>
                <h3 class="text-2xl font-black text-slate-800">{{ $totalOrganizations }}</h3>
            </div>
        </div>

        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-5 group hover:border-primary/30 transition-all cursor-default overflow-hidden">
            <div class="size-14 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 transition-transform group-hover:scale-110">
                <span class="material-symbols-outlined font-bold text-3xl">verified</span>
            </div>
            <div>
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ trans('admin/main.verified_organizations') }}</p>
                <h3 class="text-2xl font-black text-slate-800">{{ $verifiedOrganizations }}</h3>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-5 group hover:border-primary/30 transition-all cursor-default overflow-hidden">
            <div class="size-14 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 transition-transform group-hover:scale-110">
                <span class="material-symbols-outlined font-bold text-3xl">local_library</span>
            </div>
            <div>
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ trans('admin/main.organizations_instructors') }}</p>
                <h3 class="text-2xl font-black text-slate-800">{{ $totalOrganizationsTeachers }}</h3>
            </div>
        </div>

        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-5 group hover:border-primary/30 transition-all cursor-default overflow-hidden">
            <div class="size-14 rounded-2xl bg-sky-50 flex items-center justify-center text-sky-600 transition-transform group-hover:scale-110">
                <span class="material-symbols-outlined font-bold text-3xl">school</span>
            </div>
            <div>
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ trans('admin/main.organizations_students') }}</p>
                <h3 class="text-2xl font-black text-slate-800">{{ $totalOrganizationsStudents }}</h3>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ trans('admin/main.instructors') }} / {{ trans('admin/main.students') }}</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @php
                        ...
                        $nm = $user->full_name ?? 'Organization';
                    ...
                    <tr class="hover:bg-slate-50 transition-colors italic-none">
                        ...
                        <td class="px-6 py-4">
                             <div class="flex items-center gap-4">
                                <div class="flex flex-col">
                                    <span class="text-xs font-black text-slate-800">{{ $user->getOrganizationTeachers()->count() }}</span>
                                    <span class="text-[8px] font-bold text-slate-400 uppercase tracking-tighter">{{ trans('admin/main.instructors') }}</span>
                                </div>
                                <div class="w-px h-6 bg-slate-100"></div>
                                <div class="flex flex-col">
                                    <span class="text-xs font-black text-slate-800">{{ $user->getOrganizationStudents()->count() }}</span>
                                    <span class="text-[8px] font-bold text-slate-400 uppercase tracking-tighter">{{ trans('admin/main.students') }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-1 items-start">
                                <span class="px-2.5 py-1 rounded-full {{ $sc }} text-[9px] font-black uppercase tracking-widest">{{ $sl }}</span>
                                <span class="text-[8px] font-bold {{ ($user->verified ? 'text-emerald-500' : 'text-amber-500') }} uppercase tracking-tighter">({{ $user->verified ? 'Verified' : 'Unverified' }})</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                @can('admin_users_impersonate')
                                    <a href="{{ getAdminPanelUrl() }}/users/{{ $user->id }}/impersonate" target="_blank" class="size-8 rounded-lg bg-slate-50 text-slate-400 hover:text-indigo-600 transition-colors flex items-center justify-center border border-slate-100" title="Impersonate">
                                        <span class="material-symbols-outlined text-lg">user_shield</span>
                                    </a>
                                @endcan
                                ...
                                @can('admin_users_delete')
                                    @include('admin.includes.delete_button',['url' => getAdminPanelUrl().'/users/'.$user->id.'/delete' , 'btnClass' => 'size-8 rounded-lg bg-slate-50 text-slate-400 hover:text-rose-600 transition-colors flex items-center justify-center border border-slate-100', 'deleteConfirmMsg' => trans('update.user_delete_confirm_msg')])
                                @endcan
                            </div>
                        </td>
                    </tr>
    ...
    <div id="viewGrid" class="hidden">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            @php
                 ...
                 $ini = strtoupper(substr($parts[0],0,1) . (count($parts)>1?substr($parts[count($parts)-1],0,1):''));
            @endphp
            <div class="group bg-white rounded-3xl p-6 shadow-sm border border-slate-200 transition-all hover:shadow-xl hover:border-primary/20 relative flex flex-col items-center text-center overflow-hidden italic-none">
                ...
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6 px-4 truncate w-full">{{ $user->email ?? 'org@platform.com' }}</p>
                
                <div class="grid grid-cols-2 gap-3 w-full mb-6 italic-none">
                    <div class="bg-slate-50 rounded-2xl p-3 flex flex-col items-center justify-center border border-slate-100">
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-tighter mb-1">{{ trans('admin/main.instructors') }}</span>
                        <span class="text-sm font-black text-slate-800">{{ $user->getOrganizationTeachers()->count() }}</span>
                    </div>
                    <div class="bg-slate-50 rounded-2xl p-3 flex flex-col items-center justify-center border border-slate-100">
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-tighter mb-1">{{ trans('admin/main.students') }}</span>
                        <span class="text-sm font-black text-primary">{{ $user->getOrganizationStudents()->count() }}</span>
                    </div>
                </div>
    ...
    <div class="mt-8 px-6 py-5 bg-white rounded-3xl border border-slate-200 shadow-sm flex items-center justify-between">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest italic-none">
            Showing <span class="text-slate-800 font-black">{{ $users->firstItem() }}-{{ $users->lastItem() }}</span> of {{ $users->total() }}
        </p>
        <div class="um-tailwind-pagination italic-none">
    ...
    <div class="mt-12 bg-white/50 backdrop-blur rounded-3xl border border-dashed border-slate-200 p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 italic-none">
            <div class="space-y-2">
                <h6 class="text-xs font-black text-primary uppercase tracking-widest">{{ trans('admin/main.organizations_hint_title_1') }}</h6>
                <p class="text-[11px] text-slate-500 leading-relaxed font-medium">{{ trans('admin/main.organizations_hint_description_1') }}</p>
            </div>
            <div class="space-y-2">
                <h6 class="text-xs font-black text-primary uppercase tracking-widest">{{ trans('admin/main.organizations_hint_title_2') }}</h6>
                <p class="text-[11px] text-slate-500 leading-relaxed font-medium">{{ trans('admin/main.organizations_hint_description_2') }}</p>
            </div>
            <div class="space-y-2">
                <h6 class="text-xs font-black text-primary uppercase tracking-widest">{{ trans('admin/main.organizations_hint_title_3') }}</h6>
                <p class="text-[11px] text-slate-500 leading-relaxed font-medium">{{ trans('admin/main.organizations_hint_description_3') }}</p>
            </div>
        </div>
    </div>
...
<script>
    function toggleView(view) {
        ...
        if (view === 'list') {
            ...
            localStorage.setItem('umOrgView', 'list');
        } else {
            ...
            localStorage.setItem('umOrgView', 'grid');
        }
    }

    document.addEventListener('DOMContentLoaded', () => {
        const pref = localStorage.getItem('umOrgView') || 'list';
        toggleView(pref);
    });
</script>
<style>
