                        ...
                        <li data-id="{{ !empty($chapter) ? $chapter->id :'' }}" data-chapter-order="{{ $chapter->order }}" class="accordion-row bg-white rounded-sm mt-20 py-15 py-lg-30 px-10 px-lg-20">
                            <div class="d-flex align-items-center justify-content-between " role="tab" id="chapter_{{ !empty($chapter) ? $chapter->id :'record' }}">
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    @if($chapter->status != \App\Models\RemedyChapter::$chapterActive)
                                    ...
                                    <div class="btn-group dropdown table-actions">
                                        ...
                                        <div class="dropdown-menu ">
                                            <!--@if($remedy->isRemedy())-->
                                            <!--    <button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $remedy->id }}" data-type="session" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">-->
                                            <!--        {{ trans('public.add_session') }}-->
                                            <!--    </button>-->
                                            <!--@endif-->

                                            <button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $remedy->id }}" data-type="file" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">
                                                {{ trans('public.add_file') }}
                                            </button>

                                            @if(getFeaturesSettings('new_interactive_file'))
                                                <button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $remedy->id }}" data-type="new_interactive_file" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">
                                            ...
                                            <!--<button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $remedy->id }}" data-type="quiz" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">-->
                                            <!--    {{ trans('public.add_quiz') }}-->
                                            <!--</button>-->

                                            @if(getFeaturesSettings('remedy_assignment_status'))
                                                <button type="button" class="js-add-course-content-btn d-block mb-10 btn-transparent" data-webinar-id="{{ $remedy->id }}" data-type="assignment" data-chapter="{{ !empty($chapter) ? $chapter->id :'' }}">
                                    ...
                                    <button type="button" class="js-add-chapter btn-transparent text-gray" data-remedy-id="{{ $remedy->id }}" data-chapter="{{ $chapter->id }}" data-locale="{{ mb_strtoupper($chapter->locale) }}">
                                        <i data-feather="edit-3" class="mr-10 cursor-pointer" height="20"></i>
                                    </button>

                                    <a href="{{ getAdminPanelUrl() }}/rechapter/{{ $chapter->id }}/delete" class="delete-action btn btn-sm btn-transparent text-gray">
