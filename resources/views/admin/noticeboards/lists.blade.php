                ...
                <div class="card-header">
                    @can('admin_noticeboards_send')
                        <div class="text-right">
                            <a href="{{ getAdminPanelUrl() }}/{{ (!empty($isCourseNotice) and $isCourseNotice) ? 'course-noticeboards' : 'noticeboards' }}/send" class="btn btn-primary">{{trans('admin/main.send_noticeboard')}}</a>
                        </div>
                    @endcan
                </div>
                                ...
                                <tr>
                                    ...
                                    @if(!empty($isCourseNotice) and !empty($noticeboard->webinar))
                                        <td class="text-left">
                                            @if(!empty($noticeboard->webinar))
                                                <a href="{{ getAdminPanelUrl() }}/webinars/{{ $noticeboard->webinar->id }}/edit" target="_blank" class="font-14 d-block">{{ $noticeboard->webinar->id }}-{{ truncate($noticeboard->webinar->title,32) }}</a>
                                            @endif
                                        </td>
                                    @endif
                                    ...
                                    <td width="100">
                                        @can('admin_noticeboards_edit')
                                            <a href="{{ getAdminPanelUrl() }}/{{ (!empty($isCourseNotice) and $isCourseNotice) ? 'course-noticeboards' : 'noticeboards' }}/{{ $noticeboard->id }}/edit" class="btn-transparent  text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('admin/main.edit') }}">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endcan

                                        @can('admin_notifications_delete')
                                            @include('admin.includes.delete_button',['url' => getAdminPanelUrl().'/'. ((!empty($isCourseNotice) and $isCourseNotice) ? "course-noticeboards" : "noticeboards" .'/'. $noticeboard->id).'/delete','btnClass' => ''])
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
