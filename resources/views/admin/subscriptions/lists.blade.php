                    ...
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            <div class="card-header">
                                <h4>{{trans('update.total_subscriptions')}}</h4>
                            </div>
                            <div class="card-body">
                                {{ $totalSubscriptions }}
                            </div>
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            ...
                            <div class="card-body">
                                {{ !empty($totalSales->sales_count) ? $totalSales->sales_count : 0 }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                    <div class="card card-statistic-1">
                        ...
                        <div class="card-wrap">
                            ...
                            <div class="card-body">
                                {{ !empty($totalSales->total_amount) ? handlePrice($totalSales->total_amount) : 0 }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            @can('admin_webinars_export_excel')
                                <div class="text-right">
                                    <a href="{{ getAdminPanelUrl() }}/subscriptions/excel?{{ http_build_query(request()->all()) }}" class="btn btn-primary">{{ trans('admin/main.export_xls') }}</a>
                                </div>
                            @endcan
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped font-14 ">
                                    <tr>
                                        ...
                                        <th>{{trans('admin/main.subscription_count')}}</th>
                                    ...
                                    @foreach($subscriptions as $subscription)
                                        <tr class="text-center">
                                            ...
                                            <td width="18%" class="text-left">
                                                <a class="text-primary mt-0 mb-1 font-weight-bold" href="{{ $subscription->getUrl() }}">{{ $subscription->title }}</a>
                                                @if(!empty($subscription->category->title))
                                                    <div class="text-small">{{ $subscription->category->title }}</div>
                                                @else
                                                    <div class="text-small text-warning">{{trans('admin/main.no_category')}}</div>
                                                @endif
                                            </td>

                                            <td class="text-left">{{ $subscription->teacher->full_name }}</td>

                                            <td>
                                                @if(!empty($subscription->price) and $subscription->price > 0)
                                                    <span class="mt-0 mb-1">
                                                        {{ handlePrice($subscription->price, true, true) }}
                                                    </span>

                                                    @if($subscription->getDiscountPercent() > 0)
                                                        <div class="text-danger text-small font-600-bold">{{ $subscription->getDiscountPercent() }}% {{trans('admin/main.off')}}</div>
                                                    @endif
                                                @else
                                                    {{ trans('public.free') }}
                                                @endif
                                            ...
                                            <td>{{ handlePrice($subscription->sales->sum('total_amount')) }}</td>

                                            <td class="font-12">
                                                {{ $subscription->subscription_webinars_count }}
                                            </td>

                                            <td class="font-12">{{ dateTimeFormat($subscription->created_at, 'Y M j | H:i') }}</td>

                                            <td class="font-12">{{ dateTimeFormat($subscription->updated_at, 'Y M j | H:i') }}</td>
                                            ...
                                            <td>
                                                @switch($subscription->status)
                                                    ...
                                                    @break
                                                    @case(\App\Models\Subscription::$pending)
                                                    <span class="text-warning">{{ trans('admin/main.waiting') }}</span>
                                                    @break
                                                    @case(\App\Models\Subscription::$inactive)
                                                ...
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu text-left webinars-lists-dropdown">
                                                        ...
                                                        @can('admin_webinar_students_lists')
                                                            <a href="{{ getAdminPanelUrl() }}/subscriptions/{{ $subscription->id }}/students" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('admin/main.students') }}">
                                                        ...
                                                        @can('admin_support_send')
                                                            {{--<a href="{{ getAdminPanelUrl() }}/supports/create?user_id={{ $subscription->teacher->id }}" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1" title="{{ trans('admin/main.send_message_to_teacher') }}">
                                                        ...
                                                        @can('admin_webinars_edit')
                                                            <a href="{{ getAdminPanelUrl() }}/subscriptions/{{ $subscription->id }}/edit" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('admin/main.edit') }}">
                        ...
                        <div class="card-footer text-center">
                            {{ $subscriptions->appends(request()->input())->links('pagination::bootstrap-4') }}
                        </div>
