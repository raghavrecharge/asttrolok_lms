...
@section('content')
    ...
    {{-- Export progress is handled by the global tray in navbar.blade.php --}}
@endsection
...
@push('scripts_bottom')
    ...
    <script>
        $(document).ready(function() {
            ...
            // --- Core Export Logic (Background Queue) ---
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            $.toast({
                                ...
                                text: 'The export has been queued and is processing in the background. You can track progress in the download tray in the header.',
                            ...
                            // Trigger global event for navbar polling to start immediately
                            $(document).trigger('exportStarted');
                        }
                    },
                    error: function(xhr) {
                        $btn.removeClass('btn-progress').attr('disabled', false);
                        var msg = 'Failed to start export.';
                        if (xhr.responseJSON && xhr.responseJSON.error) {
                            msg = xhr.responseJSON.error;
                        }
                        $.toast({
