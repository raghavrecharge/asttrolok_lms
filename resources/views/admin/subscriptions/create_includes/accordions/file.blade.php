...
.toggle-label {
     ...
     <li data-id="{{ $subscriptionItem->id }}" id="file-item-{{ $file->id }}" data-id="{{ $file->id }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
        <div class="d-flex align-items-center justify-content-between " role="tab" id="">
            <div class="d-flex align-items-center" href="" aria-controls="collapseFile{{ !empty($file) ? $file->id :'record' }}" data-parent="#chapterContentAccordion{{ $subscriptionItems->isNotEmpty() ? $subscriptionItems->first()->id : 'record' }}"
 role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
               <i data-feather="{{ $file->file ? $file->file->getIconByType() : 'file' }}"></i>
            </span>

                            <div class="font-weight-bold text-dark-blue d-block cursor-pointer">
                {{ optional($file->file)->title ?? trans('public.add_new_files') }}
                @if(optional($file->file)->accessibility == 'free')
                    ({{ trans('public.free') }})
                @endif
            </div>
            </div>
...
<label class="custom-control custom-switch mr-20">
    ...
    <span id="status-label-{{ $file->id }}" class="custom-control-label">
        {{ $file->status == 'active' ? __('Active') : __('Inactive') }}
    </span>
</label>
                {{--@if(!empty($file))
        ...
        <div id="collapseFile{{ !empty($file) ? $file->id :'record' }}" aria-labelledby="file_{{ !empty($file) ? $file->id :'record' }}" class=" collapse @if(empty($file)) show @endif" role="tabpanel">
            <div class="panel-collapse text-gray">
                <div class="js-content-form file-form" data-action="{{ getAdminPanelUrl() }}/files/{{ !empty($file) ? $file->id . '/update' : 'store' }}">
                    <input type="hidden" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][webinar_id]" value="">
                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                >
                                    ...
                                    @endforeach
                                    <option value="secure_host">Secure Host</option>
                                    <option value="secure_host_link" @if(!empty($file) and $file->storage =='secure_host_link' ) selected @endif>Secure Host Link</option>
                                </select>
                            </div>
...
function toggleStatus(id, isChecked) {
    ...
    fetch("{{ route('subscriptionItem.status.update', ':id') }}".replace(':id', id), {
        method: "POST",
        headers: {
    ...
    .then(data => {
        ...
        if (data.title === 'Request done successfully!' || data.msg === 'Items sorted successfully') {
            ...
            Swal.fire({
                ...
                text: `The item status has been set to ${status}.`,
