...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.currentInstallmentId = null;
        ...
        validateBtn.addEventListener('click', function() {
            ...
            fetch('/admin/supports/newsuportforasttrolok/validate-offline-coupon', {
                ...
                body: JSON.stringify({
                    ...
                    installment_id: window.currentInstallmentId
                })
            })
            .then(r => r.json())
            .then(data => {
    ...
    if (installmentSelect) {
        installmentSelect.addEventListener('change', function() {
            window.currentInstallmentId = this.value ? parseInt(this.value) : null;
            updateInstallmentPreview();
    ...
    function updateInstallmentPreview() {
        ...
        if (window.currentCouponCode) {
            fetch('/admin/supports/newsuportforasttrolok/validate-offline-coupon', {
                ...
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken },
                body: JSON.stringify({
                    coupon_code: window.currentCouponCode,
                    webinar_id: webinarId,
                    cash_amount: cashAmount,
                    installment_id: window.currentInstallmentId
                })
            ...
            .then(data => {
                if (data.success && data.installment_schedules) {
                    renderSchedulePreview(data.installment_schedules, cashAmount);
                    updatePriceBreakdown(data.original_price, data.discount_amount, data.final_payable, data.remaining, data.is_sufficient, window.currentCouponCode);
                }
            });
        } else {
